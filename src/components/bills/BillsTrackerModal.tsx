...
export function BillsTrackerModal({ open, onOpenChange }: BillsTrackerModalProps) {
  const { 
    ...
    getOtherPaidExpensesForMonth, // NEW IMPORT
  } = useFinance();
  ...
  // NEW: Despesas externas pagas no mês
  const externalPaidExpenses = useMemo(() => 
            ...
            <div className="flex-1 flex flex-col min-w-0 space-y-4">
                ...
                <div className="flex-1 min-h-0">
                    <BillsTrackerList
                        ...
                        externalExpenses={externalPaidExpenses} // NEW PROP
...
<dyad-write path="src/components/bills/BillsSidebarKPIs.tsx" description="Atualizando o sidebar KPIs para incluir despesas externas">
...
export function BillsSidebarKPIs({ currentDate, totalPendingBills }: BillsSidebarKPIsProps) {
  ...
  const totalExternalExpenses = useMemo(() => 
    externalExpenses.reduce((acc, expense) => acc + expense.amount, 0)
  , [externalExpenses]);
    ...
    <div className="space-y-4 shrink-0 w-full">
      <Card className="glass-card stat-card-neutral">
        ...
        <CardContent className="p-4 pt-2 space-y-3">
          ...
          <div className="flex justify-between items-center text-sm p-2 rounded-lg bg-success/10">
            <span className="text-success font-medium flex items-center gap-2">
                <TrendingDown className="w-4 h-4" />
                Despesas Externas
            </span>
            <span className="font-bold text-success">{formatCurrency(totalExternalExpenses)}</span>
          </div>
