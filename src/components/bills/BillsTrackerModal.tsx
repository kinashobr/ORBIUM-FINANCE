...
export function BillsTrackerModal({ open, onOpenChange }: BillsTrackerModalProps) {
  ...
  const combinedBills: BillDisplayItem[] = useMemo(() => {
    ...
    const trackerPaidTxIds = new Set(
      trackerManagedBills
    ...
    const externalBills: BillDisplayItem[] = externalPaidBills.filter(
      externalBill => !trackerPaidTxIds.has(externalBill.id)
    );
  ...
  const totalUnpaidBills = useMemo(() => {
    // Filtra apenas as contas que ainda não foram pagas
  ...
  const totalPaidBills = useMemo(() => {
    // Filtra apenas as contas que foram pagas
    return combinedBills
      .filter(b => b.isPaid)
      .reduce((acc, b) => acc + b.expectedAmount, 0);
  }, [combinedBills]);
  ...
  const handleUpdateBill = useCallback(
    (id: string, updates: Partial<BillTracker>) => {
  ...
  const handleAddBill = useCallback(
    (bill: Omit<BillTracker, "id" | "isPaid" | "type">) => {
  ...
  const handleTogglePaid = useCallback(
    (bill: BillTracker, isChecked: boolean) => {
  ...
  const handleToggleFixedBill = useCallback(
    (potentialBill: PotentialFixedBill, isChecked: boolean) => {
      ...
      if (isChecked) {
        ...
        const billToRemove = billsTracker.find(
          b => b.sourceType === sourceType && b.sourceRef === sourceRef && b.parcelaNumber === parcelaNumber
        );
